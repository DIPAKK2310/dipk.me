...
export function reactToString(content: React.ReactNode): string {
  ...
  if (Array.isArray(content)) return content.map(reactToString).join("");

  if (React.isValidElement(content)) {
    // ✅ TypeScript now knows this is a ReactElement
    const children: React.ReactNode = content.props.children;
